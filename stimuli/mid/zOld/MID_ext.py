...
def initialization(expName,version):
    ...
    expInfo = {u'participant': '', u'session':'',u'run':['pre','post']}
...
FixClock = core.Clock()
# Create a new stimulus for ITI


# Initialize components for Routine "instructions"
instructPrompt = visual.TextStim(win=win, font='Arial', pos=(0, yScr/10), height=fontH, wrapWidth=wrapW, color=textCol);
instructFinish = visual.TextStim(win, text="You have reached the end of the instructions. When you are ready to begin the task, place your fingers on the keys and press your index finger again",
...
for thisTrial in trials:
    ...
    currentLoop = trials   
    print(f"Trial {trial_counter + 1} / {len(stimuli)}")  # Track trial index and stimuli size
    ...
    # Inside the cue routine, right before the while loop
    cue_start = globalClock.getTime()
    ...
    # ------Prepare to start ISI Fixation-------
    t = 0
    ISIClock = core.Clock()  # create specific clock for ISI
    routineTimer.reset()   
    continueRoutine = True
    
    # set ISI duration and record intended timing
    actual_target_duration = min_target_dur + frameDur*thisTrial - win.monitorFramePeriod * 0.75
    isi_duration = random.uniform(.5, 3)
    ITI = 3.5 - isi_duration - actual_target_duration
    nominalTime += isi_duration
    routineTimer.add(isi_duration)

    # Add intended ISI to data
    trials.addOtherData('intended_ISI', isi_duration)
    
    # Record ISI start time
    isi_start = globalClock.getTime()
    
    # -------Start ISI Fixation Routine-------
    while continueRoutine and routineTimer.getTime() > 0:
        t = ISIClock.getTime()
            
        # ISI fixation updates
        if t >= 0.0 and fix.status == NOT_STARTED:
            fix.tStart = t
            fix.setAutoDraw(True)
        frameRemains = 0.0 + isi_duration - win.monitorFramePeriod * 0.75
        ...
        for thisComponent in [fix]:  # simplified component list
            if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
                continueRoutine = True
                break
    ...
    # -------End ISI Fixation Routine-------
    fix.setAutoDraw(False)
    
    # Record actual ISI duration
    isi_end = globalClock.getTime()
    actual_isi = isi_end - isi_start
    trials.addOtherData('actual_ISI', actual_isi)
    ...
    # ------Prepare to start ITI Fixation-------
    t = 0
    ITIClock = core.Clock()  # create specific clock for ITI
    routineTimer.reset()   
    continueRoutine = True

    # Calculate ITI duration to maintain timing
    tend = globalClock.getTime()
    tcor = tend - nominalTime  # difference between actual and pre-allocated time
    iti_duration = study_times[4] - tcor  # adjust for any timing drift
    nominalTime += study_times[4]
    routineTimer.add(iti_duration)
    
    # Add intended ITI to data
    trials.addOtherData('intended_ITI', iti_duration)
    
    # Record ITI start time
    iti_start = globalClock.getTime()
    
    # -------Start ITI Fixation Routine-------
    while continueRoutine and routineTimer.getTime() > 0:
        t = ITIClock.getTime()
            
        # ITI fixation updates
        if t >= 0.0 and fix.status == NOT_STARTED:
            fix.tStart = t
            fix.setAutoDraw(True)
        frameRemains = 0.0 + iti_duration - win.monitorFramePeriod * 0.75
        if fix.status == STARTED and t >= frameRemains:
            fix.setAutoDraw(False)
    ...
    # -------End ITI Fixation Routine-------
    fix.setAutoDraw(False)
    
    # Record actual ITI duration
    iti_end = globalClock.getTime()
    actual_iti = iti_end - iti_start
    trials.addOtherData('actual_ITI', actual_iti)
    ...
    trials.addOtherData('time.plannedfixlength', isi_duration)
...
# After the task ends, record the end time
task_end_time = globalClock.getTime()  # Record end time of the task
...
thisExp.addData('Total_Task_Duration', total_task_duration)
