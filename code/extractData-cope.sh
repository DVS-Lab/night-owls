...
if [[ "$space_tag" == "mni" ]]; then
  ...
  # COPE / VARCOPE paths corresponding to this zstat
  local statsdir="$(dirname "$zimg")"
  local cope_img="${statsdir}/cope${znum}.nii.gz"
  local varcope_img="${statsdir}/varcope${znum}.nii.gz"

  # VS means for COPE and VARCOPE
  local vs_mean_cope="NA"
  local vs_mean_varcope="NA"
  [[ -f "$cope_img"    && -f "$vs_res" ]] && vs_mean_cope="$(fslstats "$cope_img" -k "$vs_res" -M 2>/dev/null || echo NA)"
  [[ -f "$varcope_img" && -f "$vs_res" ]] && vs_mean_varcope="$(fslstats "$varcope_img" -k "$vs_res" -M 2>/dev/null || echo NA)"

  # Whole-brain spatial correlations with BRS (masked to FEAT brainmask)
  local brs_corr_cope="NA"
  local brs_corr_varcope="NA"
  if [[ -f "$brs_res" ]]; then
    [[ -f "$cope_img"    ]] && brs_corr_cope="$(fslcc -m "$brainmask" --noabs -t -1 -p 6 "$cope_img" "$brs_res" 2>/dev/null | awk '{print $NF}' || echo NA)"
    [[ -f "$varcope_img" ]] && brs_corr_varcope="$(fslcc -m "$brainmask" --noabs -t -1 -p 6 "$varcope_img" "$brs_res" 2>/dev/null | awk '{print $NF}' || echo NA)"
  fi

  fi

  echo -e "${sub}\t${ses}\t${run}\t${task}\t${space_tag}\t${acq}\t${confounds}\t${znum}\t${label}\t${vs_mean}\t${brs_corr}\t${vs_mean_cope}\t${brs_corr_cope}\t${vs_mean_varcope}\t${brs_corr_varcope}"
}

# -------- main --------
out="${OUT_DIR}/extractions_L1stats-revised_with-cope-varcope.tsv"
echo -e "sub\tses\trun\ttask\tspace\tacq\tconfounds\tzstat\tlabel\tVS_mean\tBRS_corr\tVS_mean_cope\tBRS_corr_cope\tVS_mean_varcope\tBRS_corr_varcope" > "$out"
