...
mkdir(outfiles);
subs = [101,102,103,104,105,106,107,108,109,110,999]

for s = subs

    subout = fullfile(outfiles,sprintf('sub-%0d',s));
    mkdir(subout);
    ...
    for sess = 1:sessions 
        for r = 1:runs
            ...
            fname = fullfile(subout, sprintf('sub-%d_ses-%d_run-%d_design.csv', s, sess, r));
            fid = fopen(fname,'w');
